...
{
    static LFNetworking *handler = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        handler = [[LFNetworking alloc] init];
    });
    return handler;
}
...
+(LFURLSessionTask *)baseRequestType:(NSUInteger)type
                                 url:(NSString *)url
                              params:(NSDictionary *)params
                             success:(LFResponseSuccess)success
                                fail:(LFResponseFail)fail
    ...
    LFURLSessionTask *sessionTask=nil;
...
+(LFURLSessionTask *)uploadWithImage:(UIImage *)image
    ...
    LFURLSessionTask *sessionTask = [manager POST:urlStr parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        //压缩图片
    ...
    [mgr setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status)
        {
            case AFNetworkReachabilityStatusUnknown: // 未知网络
                DLog(@"未知网络");
                [LFNetworking sharedLFNetworking].networkStats=StatusUnknown;
                
                break;
            case AFNetworkReachabilityStatusNotReachable: // 没有网络(断网)
                DLog(@"没有网络");
                [LFNetworking sharedLFNetworking].networkStats=StatusNotReachable;
                break;
            case AFNetworkReachabilityStatusReachableViaWWAN: // 手机自带网络
                DLog(@"手机自带网络");
                [LFNetworking sharedLFNetworking].networkStats=StatusReachableViaWWAN;
                break;
            case AFNetworkReachabilityStatusReachableViaWiFi: // WIFI
                
                [LFNetworking sharedLFNetworking].networkStats=StatusReachableViaWiFi;
                DLog(@"WIFI--%d",[LFNetworking sharedLFNetworking].networkStats);
                break;
        }
    }];
    [mgr startMonitoring];
...
+(NSString *)strUTF8Encoding:(NSString *)str{
    //return [str stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLPathAllowedCharacterSet]]; // iOS9
